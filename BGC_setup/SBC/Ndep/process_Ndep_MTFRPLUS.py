...
# Load Dataset
fdir = '/work/n01/n01/dapa/SANH/RAW_DATA/SBC/SANH-Ndep/DIETS/'
ds = xr.open_dataset(fdir+'SANH_Ndep_nhx_2021-2035.nc',decode_times=False)
ds['N4_flux'] = ds.wet + ds.dry

ds_temp = xr.open_dataset(fdir+'SANH_Ndep_noy_2021-2035.nc',decode_times=False)
ds['N3_flux'] = ds_temp.wet + ds_temp.dry
ds = ds[['N3_flux','N4_flux']]

ds['time'] = np.array([datetime(2004,1,15) + relativedelta(months=i) for i in ds.time])
...
for year,idx in ds.groupby('time.year').groups.items():
    ofile=f'/work/n01/n01/dapa/SANH/INPUTS/SBC/BGC/MTFRPLUS/Ndep_y%s.nc' %year
    ds.isel(time=idx).to_netcdf(ofile,unlimited_dims='time')
    if year == 2018:
        ofile=f'/work/n01/n01/dapa/SANH/INPUTS/SBC/BGC/MTFR/Ndep_y2019.nc'
        ds_tmp = ds.isel(time=idx)
        ds_tmp['time'] = np.array([datetime(2019,1,15) + relativedelta(months=i) for i in range(12)]) 
        ds_tmp.to_netcdf(ofile,unlimited_dims='time')
